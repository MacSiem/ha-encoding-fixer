/* HA Tools split — ha-encoding-fixer v5.0.13 (2026-08-28) — single-tool standalone repo */
(function() {
'use strict';

// Component-local persistence retains this card's existing localStorage keys.
const haToolsPersistence = { _cache: {}, _hass: null, setHass(h) { this._hass = h; }, async save(k, d) { try { localStorage.setItem('ha-encoding-fixer-' + k, JSON.stringify(d)); } catch(e) { console.debug('[ha-encoding-fixer] caught:', e); } }, async load(k) { try { const r = localStorage.getItem('ha-encoding-fixer-' + k); return r ? JSON.parse(r) : null; } catch(e) { return null; } }, loadSync(k) { try { const r = localStorage.getItem('ha-encoding-fixer-' + k); return r ? JSON.parse(r) : null; } catch(e) { return null; } } };
...
const HA_ENCODING_FIXER_BENTO_CSS = `
/* ═══════════════════════════════════════════════
...
class HaEncodingFixer extends HTMLElement {
  constructor() {
    ...
    this._integrationAvailable = null;
    this._integrationHint = '';
    this._lastIntegrationChanges = [];
    this._serverBackups = [];
  ...
  _applyIntegrationScanResult(result, source) {
    const changes = this._normalizeDryRunChanges(result?.changes || []);
    this._lastIntegrationChanges = changes;

    const entityChanges = changes.filter(change => change.type === 'entity_registry');
    this._scanResults = entityChanges.map(change => ({
      entity_id: change.entity_id,
      attribute: change.attribute || 'friendly_name',
      original: change.before,
      fixed: change.after,
      method: change.method || 'server-scan',
      _change: change
    }));
    this._scanTotal = entityChanges.length;

    const fileChanges = changes.filter(change => change.type === 'file');
    this._yamlResults = {
      timestamp: Math.floor(Date.now() / 1000),
      mode: 'integration_scan',
      scanned_files: result?.scanned_files || 0,
      total_issues: fileChanges.length,
      fixed_files: 0,
      errors: result?.errors || [],
      issues: fileChanges.map(change => ({
        file: change.file,
        line: change.line,
        issue: 'mojibake',
        detail: this._lang === 'pl' ? 'Wykryto mojibake/BOM' : 'Detected mojibake/BOM',
        context: change.before + ' \u2192 ' + change.after,
        fixable: true,
        via: 'integration',
        change
      }))
    };
    if (source === 'entities') this._activeTab = 'scan';
    if (source === 'yaml') this._activeTab = 'yaml';
  }

  async _scanWithIntegration(source) {
    const isEntityScan = source === 'entities';
    if (!this._hass) return false;
    if (isEntityScan && this._scanning) return true;
    if (!isEntityScan && this._yamlScanning) return true;
    if (isEntityScan) {
      this._scanning = true;
    ...
    this._updateUI();
    try {
      const result = await this._callEncodingFixerWS({ type: 'ha_encoding_fixer/scan' }, { fallbackAllowed: true });
      if (!result) return false;
      this._applyIntegrationScanResult(result, source);
      return true;
    } catch (err) {
      console.warn('[Encoding Fixer] integration scan error:', err);
      this._showToast((this._lang === 'pl' ? 'Blad skanowania integracji: ' : 'Integration scan error: ') + (err.message || err), 'error');
      return true;
    } finally {
      this._scanning = false;
      this._entityScanning = false;
  ...
  // --- Scan entities ---
  async _scanEntities() {
    const handled = await this._scanWithIntegration('entities');
    if (handled) return;
    return this._scanEntitiesLegacy();
  }

  async _scanEntitiesLegacy() {
  ...
  async _scanLovelace() {
    ...
    try {
      ...
      if (haToolsPersistence) {
        haToolsPersistence.setHass(this._hass);
        const snap = await haToolsPersistence.load('encoding-fixer-resource-snapshot');
  ...
  async _fixSelectedEntities() {
    ...
    if (!entitiesToFix.length) return;
    const changes = entitiesToFix.map(r => r._change).filter(Boolean);
    if (changes.length === entitiesToFix.length) {
      const handled = await this._confirmAndApplyChanges(
        changes,
        this._lang === 'pl' ? 'Napraw zaznaczone encje' : 'Fix selected entities',
        async () => { await this._scanWithIntegration('entities'); }
      );
      if (handled) return;
    }
  ...
  async _fixAllEntities() {
    ...
    if (changes.length === entitiesToFix.length) {
      const handled = await this._confirmAndApplyChanges(
        changes,
        this._lang === 'pl' ? 'Napraw wszystkie encje' : 'Fix all entities',
        async () => { await this._scanWithIntegration('entities'); }
      );
      if (handled) return;
    }
  ...
  _isIntegrationMissingError(err) {
  ...
  _buildIntegrationHint() {
    if (this._integrationAvailable !== false) return '';
    const msg = this._integrationHint || (this._lang === 'pl'
      ? 'Tryb legacy: zainstaluj integracje Encoding Fixer, aby wlaczyc backupi serwerowe i restore.'
      : 'Legacy mode: install and configure the Encoding Fixer integration to enable server backups and restore.');
    return `<div class="section info-section integration-hint">\u26A0\uFE0F ${this._escapeHtml(msg)}</div>`;
  }

  _normalizeDryRunChanges(changes) {
  ...
  _showChangeDialog(changes, title) {
    const sr = this.shadowRoot;
    if (!sr) return Promise.resolve(false);
    const normalized = this._normalizeDryRunChanges(changes);
    const shown = normalized.slice(0, 80);
    return new Promise(resolve => {
      const existing = sr.querySelector('.confirm-overlay');
      if (existing) existing.remove();
      const overlay = document.createElement('div');
      overlay.className = 'confirm-overlay';
      const rows = shown.map(change => `
        <div class="confirm-row">
          <div class="confirm-file">${this._escapeHtml(change.entity_id || change.file || '')}${change.line ? ':' + change.line : ''}</div>
          <div class="confirm-before">${this._escapeHtml(change.before)}</div>
          <div class="confirm-arrow">\u2192</div>
          <div class="confirm-after">${this._escapeHtml(change.after)}</div>
        </div>
      `).join('');
      overlay.innerHTML = `
        <div class="confirm-dialog" role="dialog" aria-modal="true">
          ...
          <p>${this._lang === 'pl'
            ? 'To jest wynik dry-run. Nic nie zostalo zapisane. Zastosowanie utworzy timestamped backup przed kazdym zapisem.'
            : 'This is a dry run. Nothing has been written. Applying will create a timestamped backup before every write.'}</p>
          <div class="confirm-count">${normalized.length} ${this._lang === 'pl' ? 'zmian' : 'changes'}</div>
          <div class="confirm-list">${rows}</div>
          ${normalized.length > shown.length ? `<div class="confirm-more">+${normalized.length - shown.length} ${this._lang === 'pl' ? 'wiecej' : 'more'}</div>` : ''}
          <div class="confirm-actions">
            <button class="btn btn-secondary" data-dialog-cancel>${this._lang === 'pl' ? 'Anuluj' : 'Cancel'}</button>
            <button class="btn btn-danger" data-dialog-apply>${this._lang === 'pl' ? 'Zastosuj naprawde' : 'Apply for real'}</button>
          </div>
        </div>`;
      ...
      overlay.querySelector('[data-dialog-apply]')?.addEventListener('click', () => close(true));
  ...
  async _confirmAndApplyChanges(changes, title, afterApply) {
    try {
      const dryRun = await this._callEncodingFixerWS({
        type: 'ha_encoding_fixer/fix',
        dry_run: true,
        changes
      }, { fallbackAllowed: true });
      if (!dryRun) return false;
      const dryRunChanges = this._normalizeDryRunChanges(dryRun.changes || []);
      if (!dryRunChanges.length) {
        this._showToast(this._lang === 'pl' ? 'Brak aktualnych zmian do zastosowania' : 'No current changes to apply', 'info');
        return true;
      }
      const confirmed = await this._showChangeDialog(dryRunChanges, title);
      if (!confirmed) return true;
      const result = await this._callEncodingFixerWS({
        type: 'ha_encoding_fixer/fix',
        dry_run: false,
        changes: dryRunChanges
      });
      const failed = result.failed || 0;
      this._addFixLog('integration', title, failed ? 'failed' : 'success', 'backup ' + (result.backup_id || ''));
      this._showToast(
        failed
          ? (this._lang === 'pl' ? 'Zastosowano z bledami: ' : 'Applied with errors: ') + failed
          : (this._lang === 'pl' ? 'Zastosowano. Backup: ' : 'Applied. Backup: ') + result.backup_id,
        failed ? 'error' : 'success'
      );
      if (afterApply) await afterApply(result);
      return true;
    } catch (err) {
  ...
  _buildHTML() {
    const t = this._t;
    return `<style>${HA_ENCODING_FIXER_BENTO_CSS}
...
:host(.bento-dark) .intro-banner .intro-dismiss:hover { background: #27272f; color: #fafaf9; }
  ...
  _attachEvents() {
    ...
    const listBackups = sr.querySelector('[data-action="list-backups"]');
    if (listBackups) listBackups.addEventListener('click', () => this._loadServerBackups());
    sr.querySelectorAll('[data-restore-backup]').forEach(btn => {
      btn.addEventListener('click', () => this._restoreServerBackup(btn.dataset.restoreBackup));
    });
  ...
  async _scanViaApi() {
    const handled = await this._scanWithIntegration('yaml');
    if (handled) return;
    return this._scanViaApiLegacy();
  }

  async _scanViaApiLegacy() {
  ...
  async _fixYamlFile(idx) {
    ...
    if (issue.via === 'integration' && issue.change) {
      await this._confirmAndApplyChanges(
        [issue.change],
        this._lang === 'pl' ? 'Napraw plik konfiguracyjny' : 'Fix config file',
        async () => { await this._scanWithIntegration('yaml'); }
      );
      return;
    }
  ...
  async _fixYamlAll() {
    ...
    const integrationIssues = (this._yamlResults.issues || []).filter(i => i.via === 'integration' && i.fixable && i.change);
    if (integrationIssues.length > 0) {
      await this._confirmAndApplyChanges(
        integrationIssues.map(i => i.change),
        this._lang === 'pl' ? 'Napraw wszystkie pliki' : 'Fix all config files',
        async () => { await this._scanWithIntegration('yaml'); }
      );
      return;
    }
  ...
  _buildRestoreTab() {
    ...
    const backupRows = (this._serverBackups || []).map(backup => `
      <div class="backup-row">
        ...
        <div class="backup-files">${backup.file_count || 0} ${this._lang === 'pl' ? 'plikow' : 'files'}</div>
    ...
    const serverRestore = isAdmin ? `
      <div class="section">
        ...
        <p class="section-desc">${this._lang === 'pl'
          ? 'Backupi sa tworzone automatycznie przed kazdym zapisem integracji.'
          : 'Backups are created automatically before every integration write.'}</p>
        <div class="scan-header" style="margin-top:12px">
          <button class="btn btn-secondary" data-action="list-backups" ${this._backupsLoading ? 'disabled' : ''}>${this._backupsLoading ? t.restoreScanning : (this._lang === 'pl' ? 'Odswiez backupy' : 'Refresh backups')}</button>
        </div>
  ...
  async _scanForMissingResources() {
    ...
    try {
      ...
      if (haToolsPersistence) {
        haToolsPersistence.setHass(this._hass);
        backup = await haToolsPersistence.load('encoding-fixer-resource-snapshot');
      }
  ...
  async _createResourceSnapshot() {
    ...
    try {
      ...
      if (haToolsPersistence) {
        haToolsPersistence.setHass(this._hass);
        await haToolsPersistence.save('encoding-fixer-resource-snapshot', snapshot);
      }
  ...
  async _restoreServerBackup(backupId) {
    ...
    try {
      const result = await this._callEncodingFixerWS({
        type: 'ha_encoding_fixer/restore',
        backup_id: backupId
      });
      this._addFixLog('restore', backupId, 'success', 'rollback ' + (result.rollback_backup_id || ''));
      this._showToast(this._t.restoreDone + ' (' + (result.results || []).length + ')', 'success');
...
.backup-id { font-family: monospace; font-weight: 600; flex: 1; min-width: 0; overflow-wrap: anywhere; }
.backup-files { color: var(--bento-text-secondary); font-size: 12px; flex-shrink: 0; }
...
.confirm-dialog p { color: var(--bento-text-secondary); font-size: 13px; line-height: 1.45; }
.confirm-count { font-size: 12px; font-weight: 700; color: var(--bento-primary); }
.confirm-list { overflow: auto; border: 1px solid var(--bento-border); border-radius: var(--bento-radius-xs); }
.confirm-row {
  display: grid;
  grid-template-columns: minmax(120px, 1fr) minmax(120px, 1fr) 24px minmax(120px, 1fr);
  gap: 8px;
  padding: 8px 10px;
  border-bottom: 1px solid var(--bento-border);
  font-size: 12px;
...
.confirm-file { font-family: monospace; color: var(--bento-text-secondary); overflow-wrap: anywhere; }
.confirm-before { color: var(--bento-error); font-family: monospace; overflow-wrap: anywhere; }
.confirm-after { color: var(--bento-success); font-family: monospace; overflow-wrap: anywhere; }
.confirm-arrow { text-align: center; color: var(--bento-text-secondary); }
.confirm-more { color: var(--bento-text-secondary); font-size: 12px; }
